...
class FileStorage : public IStorage {
    ...
    public:
        ...
        static std::vector<unsigned int> splitToInt(const std::string& str, char delimiter);

        // Write the movie IDs to the file, separated by commas: `movie1,movie2,...lastmovie`
        static void writeMoviesToFile(std::ofstream& file, std::vector<unsigned int> movies);

        StatusCode updateUserData(unsigned int userId, std::vector<unsigned int>& movies, Change change) override;

        std::vector<long long> isUserInStorage(unsigned int userId) override;
        ...
        std::vector<unsigned int> filterMovies(unsigned int userId, const std::vector<unsigned int>& movies, Change change);
};