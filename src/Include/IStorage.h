...
class IStorage {
    public:
        ...
        virtual StatusCode updateUserData(unsigned int userId, std::vector<unsigned int>& movies, Change change) = 0;
        ...
        // in addition to any unsigned integer we chose to return long long, which contains both.
        virtual std::vector<long long> isUserInStorage(unsigned int userId) = 0;
};