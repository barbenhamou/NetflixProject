...
TEST(DeleteCommandTests, RemoveFunction) {
    ...
    for (int i = 0; i < numTests; i++) {    
        ...
        unsigned int userId = randInt(10 * i, 10 * (i + 1) - 1);
        ...
        std::vector<unsigned int> moviesToRemove(movieIds.begin(), movieIds.begin() + numToRemove);
        ...
        std::vector<unsigned int> remainingMovieIds;
        ...
        std::vector<unsigned int> expectedRemaining;
