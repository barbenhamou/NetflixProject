...
TEST(MovieTests, RelativeComplement) {
    ...
    for (int i = 1; i < 21; i++) {
        ...
        EXPECT_TRUE(compareVec(correct1, result1));
        EXPECT_TRUE(compareVec(correct2, result2));
    }

    // Edge cases: A\B - 1. B=emptySet 2. A=emptySet
    auto empty = Movie::relativeComplement({}, vectors[1]);
    EXPECT_TRUE(compareVec(empty, {}));

    auto full = Movie::relativeComplement(vectors[0], {});
    EXPECT_TRUE(compareVec(full, vectors[0]));
}